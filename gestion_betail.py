...
def svg(name, size=18, color="currentColor"):
    ...
    icons = {
        ...
        "lock":      f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0110 0v4"/></svg>',
        "pencil":    f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><path d="M17 3a2.828 2.828 0 114 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>',
        "refresh":   f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><polyline points="23 4 23 10 17 10"/><path d="M20.49 15a9 9 0 11-2.12-9.36L23 10"/></svg>',
        "hourglass": f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><path d="M5 3h14v5a3 3 0 01-3 3 4 4 0 00-4 4v4h-4v-4a4 4 0 00-4-4 3 3 0 01-3-3V3z"/><path d="M5 21h14v-5a3 3 0 00-3-3 4 4 0 01-4-4v-4h-4v4a4 4 0 01-4 4 3 3 0 00-3 3v5z"/></svg>',
        "recycle":   f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><path d="M3 12l4-7 4 7"/><path d="M12 21l4-7 4 7"/><path d="M7.5 15h10"/></svg>',
        "search":    f'<svg width="{s}" height="{s}" fill="none" stroke="{color}" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>',
    }
...
def sync_animals():
    ...
    if not animals:
        st.warning("⚠️ Sync annulée : liste vide, aucune donnée envoyée au Sheet.")
        return
    try:
...
# ══════════════════════════ PAGE FICHE ANIMAL ══════════════════════════
# (inchangée, je la garde pour la continuité)
def page_animal_detail(animal_id):
    ...
    if st.button(f"{'←'} Retour au catalogue", key="back_to_catalogue"):
        if is_editing:
    ...
    with info_col:
        if is_editing:
            st.markdown(f"""<div style="font-weight:800;font-size:22px;margin-bottom:14px;">
                ✏️ Modifier {a["earTag"]}</div>""", unsafe_allow_html=True)
            c1,c2 = st.columns(2)
            with c1:
            ...
            with ec1:
                if st.button("✏️ Modifier", use_container_width=True, key="fiche_edit"):
                    st.session_state.editing_fiche_id = a["id"]
                    st.rerun()
            with ec2:
                if st.button("🗑️ Supprimer", use_container_width=True, key="fiche_delete"):
        ...
        with tab_login:
            ...
            if st.button("Se connecter", use_container_width=True, key="btn_login"):
                ...
                if not user:
                    ...
                    st.error("❌ Votre demande d'accès a été refusée. Contactez un administrateur.")
                else:
                    st.session_state.auth  = {"name": user["name"], "role": user["role"]}
            ...
            if st.button("📝 Créer mon compte", use_container_width=True, key="btn_register"):
                ...
                if errors:
                    ...
                    try:
                        ...
                        st.success("✅ Compte créé ! Un administrateur va valider votre accès sous peu.")
...
def page_catalogue():
    ...
    # ── Grille 3 colonnes ──
    # ── Ajoutez ces classes dans le <style> de page_catalogue (avec .cat-header etc.) ──
    # .cat-card-img-wrap { position:relative; background:#F5F5F5; border-radius:2px;
    #     overflow:hidden; aspect-ratio:3/4; display:flex; align-items:center;
    #     justify-content:center; margin-bottom:8px; }
    # .cat-card-img-wrap img { width:100%; height:100%; object-fit:cover; }
...
def page_animals():
    ...
    if st.session_state.show_race and not is_obs:
        st.markdown(f'<div style="background:{ACCENT_DARK};border-radius:14px 14px 0 0;padding:14px 20px;color:#fff;font-weight:700;font-size:15px;">{svg("race",18,"#fff")} Ajouter une nouvelle race</div>', unsafe_allow_html=True)
                ...
                if new_race.strip():
                    ...
                    if new_race.strip() not in liste:
                        ...
                        try:
                            ...
                            st.success(f"Race « {new_race.strip()} » ajoutée !")
                        except Exception as e:
...
def photo_uploader_block(key_prefix, current_photos):
    ...
    if photos:
        ...
        for i, col in enumerate(del_cols[:len(photos)]):
            with col:
                if st.button(f"🗑 Photo {i+1}", key=f"del_photo_{key_prefix}_{i}",
        ...
        if st.session_state.get(hash_key) != file_hash:
            with st.spinner("Upload vers Supabase…"):
                try:
                    ...
                    st.success(f"✅ Photo uploadée sur Supabase !")
                    st.rerun()
                except Exception as e:
...
def page_sales():
    ...
    if st.session_state.show_vente:
        ...
        if not dispos: st.warning("Aucun animal disponible.")
            ...
            st.markdown(f'<div style="background:{ACCENT_DARK};border-radius:14px 14px 0 0;padding:14px 20px;color:#fff;font-weight:700;font-size:15px;">{svg("sell",18,"#fff")} Enregistrer une vente</div>', unsafe_allow_html=True)
            ...
            with vc3:
                ...
                if st.button("✅ Confirmer",use_container_width=True,key="confirm_vente"):
                    ...
                    sync_animals()
                    st.success(f"Vente : {v_tag} → {fmt(v_prix)}")
...
def page_users():
            ...
            with st.container():
                ...
                with ac2:
                    if st.button("✅ Accepter", key=f"accept_{u['email']}", use_container_width=True):
                        ...
                        st.session_state.users = users
                        st.success(f"✅ {u['name']} accepté en tant que {role_sel}")
                        st.rerun()
                with ac3:
                    if st.button("❌ Refuser", key=f"refuse_{u['email']}", use_container_width=True):
                        ...
                        db.save_all_users(users)
                        st.session_state.users = users
                        st.warning(f"❌ {u['name']} refusé.")
                        st.rerun()
                with ac4:
                    if st.button("🗑️ Supprimer", key=f"del_pending_{u['email']}", use_container_width=True):
    ...
    for u in actifs:
        ...
        if is_admin:
            ...
            with ec3:
                if not is_me:
                    if st.button("🗑️ Supprimer", key=f"del_{u['email']}", use_container_width=True):
                        users = [x for x in users if x["email"] != u["email"]]
                        db.save_all_users(users)
                        st.session_state.users = users
                        st.rerun()
    ...
    if is_admin and refuses:
        ...
        with st.expander(f"❌ Comptes refusés ({len(refuses)})"):
            for u in refuses:
                ...
                with rc2:
                    if st.button("♻️ Réactiver", key=f"reactivate_{u['email']}", use_container_width=True):
                        for x in users:
                            if x["email"] == u["email"]:
                                x["statut"] = "Actif"; break
                        db.save_all_users(users)
                        st.session_state.users = users
                        st.success(f"{u['name']} réactivé.")
                        st.rerun()
                with rc3:
                    if st.button("🗑️ Supprimer", key=f"del_refused_{u['email']}", use_container_width=True):
                        users = [x for x in users if x["email"] != u["email"]]
                        db.save_all_users(users)
                        st.session_state.users = users
                        st.rerun()
...
def page_settings():
    ...
    if not is_obs:
        if st.button("💾 Enregistrer"): st.success("Paramètres enregistrés.")
...
def main():
    ...
    with cb:
        if auth["role"]=="Observateur":
            st.markdown(f'<div style="background:#E3F0FF;color:#1565C0;border-radius:20px;padding:5px 14px;font-size:11px;font-weight:600;text-align:center;margin-top:10px;">{svg("eye",13,"#1565C0")} Mode lecture</div>', unsafe_allow_html=True)
